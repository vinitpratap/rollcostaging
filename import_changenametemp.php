...
//code start for import Profile Change
            $firstName = '';
            $lastName = '';
            $customerID = addslashes(trim($data[3]));
            $CompanyName = addslashes(trim($data[0]));
            $PostCode = addslashes(trim($data[1]));
            $county = addslashes(trim($data[2]));
            ...
            if ($CompanyName != '') {
                ...
                if (isset($nameArr[1]) && $nameArr[1] != '') {
                    $lastName = substr(strstr($CompanyName, " "), 1);
                }
            }
            ...
            if ($customerID != "") {
                $sqlsts = "select u_id from rollco_ms_tmpusers where customerID='" . $customerID . "' AND com_zipCode='" . $PostCode . "'";
...
//exit;
    ...
    echo "<script>location.href='import_changenametemp.php?inscnt=" . $insertcount . "&updcnt=" . $updatecount . "';</script>";
                                ...
                                <td> <table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0" >
                                        <tr> 
                                            <td> <table width="100%%" border="0" cellspacing="0" cellpadding="0">
                                                    <tr> 
                                                        <td height="400" align="left" valign="top"><form action="import_changenametemp.php" method="post" enctype="multipart/form-data" name="form1" class="text09" onSubmit="return validate_import();">
                                                                    ...
                                                                    <tr>
                                                                        <td colspan="2" class="txt11"><span>
                                                                                ...
                                                                                <?php if (isset($_GET['inscnt']) || isset($_GET['updcnt'])) { ?>
                                                                                    ...
                                                                                    <br /><br /><br />
                                                                                    <a style="color: #000" href="import_changenametemp.php">Back to Import User</a>
                                                                                    <?php
                                                                                } else
