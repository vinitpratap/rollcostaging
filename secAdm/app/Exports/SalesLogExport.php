...
class SalesLogExport implements FromCollection, WithHeadings {
    ...
    public function collection() {
        ...
        $query = DB::table('rollco_salescallog')
                ->select('cdate', 'full_name', 'u_id', 'post_code', 'sc_country', 'sc_stime', 'sc_etime', 'sc_remarks', 'sc_status', 'sec_id', 'log_action', 'reason_delete');
        ...
        if (!empty($from) && !empty($to)) {
            ...
            $query = $query->whereDate('log_date', '>=', $from)->whereDate('log_date', '<=', $to);
        }
    ...
    public function headings(): array {
        return [
            "Date", "Name", "Account Code", "Post Code", "County/Town", "Start Time", "End Time", "Remarks", "Status", "Sales Person", "Log Action", "Reason if Delete"
        ];
    }
