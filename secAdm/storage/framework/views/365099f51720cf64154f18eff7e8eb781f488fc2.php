...
<?php $i = 1; ?>
                            ...
                            <tr class="pointer">
                                ...
                                <td><?php if(isset($value->getUserDetails[0])) { ?> <?php echo e($value->getUserDetails[0]->companyName); ?>  <?php } ?></td>
                                <td><?php if(isset($value->getUserDetails[0])) { ?><?php echo e($value->getUserDetails[0]->com_emailAddress); ?> <?php } ?></td>
                                <td><?php if(isset($value->getUserDetails[0])) { ?><?php echo e(html_entity_decode(getUserCurrency($value->getUserDetails[0]->g_id)) .' '.$value->totalprice); ?>  <?php } ?></td>
                                <td><?php echo e(countOrderProduct($value->order_id)); ?> Parts - <?php echo e($value->Qty); ?> Qty </td>

                                <td><?php if($value->order_status == 0): ?> Open <?php elseif($value->order_status == 1): ?> Processing <?php elseif($value->order_status == 2): ?> Pending <?php elseif($value->order_status == 3): ?> Hold <?php elseif($value->order_status == 4): ?> Complete <?php elseif($value->order_status == 5): ?> Closed <?php elseif($value->order_status == 6): ?> Canceled <?php endif; ?></td>
                                ...
                                <td class="last">
									...
									<a href="<?php echo e(route('order.delete',base64_encode($value->order_id))); ?>"  title="Delete" class="d-inline-block delete confirmation"><img src="<?php echo e(URL::asset('images/delete.svg')); ?>" alt=""></a>
									<?php } ?>
                                </td>
                            </tr>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
<?php if (count($data) > 0) { ?>
                <div class="row  m-0">
                    ...
                    <div class="col-6 text-right pl-0 pr-0 pb-3 clearfix"><a href="<?php echo e(route('order.export',$routeValues)); ?>" class="btn btn-outline-secondary pl-4 pr-4 pt-2 pb-2  m-0" id="exporttoExcel"> <img src="<?php echo e(URL::asset('images/excel.svg')); ?>" class="pr-2" alt="Export to Excel"> Export Order </a></div>
				 ...
				 <div class="col-6"></div>
                    <?php $routeValues = 'search=' . app('request')->input('search'). '&search_user=' . app('request')->input('search_user') . '&ord_status=' . app('request')->input('ord_status') . '&daterange=' . app('request')->input('req_date_range'); ?>
                    <div class="col-6 text-right pl-0 pr-0 pb-3 clearfix"><a href="<?php echo e(route('orderunitwise.export',$routeValues)); ?>" class="btn btn-outline-secondary pl-4 pr-4 pt-2 pb-2  m-0" id="exporttoExcel"> <img src="<?php echo e(URL::asset('images/excel.svg')); ?>" class="pr-2" alt="Export to Excel"> Export Unit Wise Order </a></div>
